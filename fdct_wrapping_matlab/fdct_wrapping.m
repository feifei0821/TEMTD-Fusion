...
%
% Optional Inputs
%   is_real     Type of the transform 变换类型
%                   0: complex-valued curvelets 复值曲线
%                   1: real-valued curvelets 实值曲线
%               [default set to 0]
...
%                   2: wavelets 小波
%               [default set to 2]
%   nbscales    number of scales including the coarsest wavelet level 尺度数
%               [default set to ceil(log2(min(M,N)) - 3)]
%   nbangles_coarse
%               number of angles at the 2nd coarsest level, minimum
%               8,第二粗糙等级的角度数，最小为8
%               must be a multiple of 4. [default set to 16] 4的倍数
%
% Outputs
%   C           Cell array of curvelet coefficients.曲线波系数的单元阵列
%               C{j}{l}(k1,k2) is the coefficient at
%                   - scale j: integer, from finest to coarsest
%                   scale,整数，从最细到最粗
%                   - angle l: integer, starts at the top-left corner and
%                   increases clockwise,整数，从左上角开始，顺时针增加
...
% Initialization: data structure 初始化：数据结构
...
% Loop: pyramidal scale decomposition 金字塔尺度分解
M1 = N1/3;
M2 = N2/3;
if finest == 1,
    ...
    X = X(equiv_index_1,equiv_index_2);
        ...
        % is the center in frequency. Same for M2, N2.频率中心
